...
func menuInput() {
	fmt.Println("---------------------")
	fmt.Println("	1. Input Pabrikan")
	fmt.Println("	2. Input Mobil")
	fmt.Println("	3. Kembali")
	fmt.Println("---------------------")
	fmt.Print("Pilih Menu: ")
}
func inputMobil(a int, P *pabrikan) {
	if a == 0 {
		fmt.Println("---------------------")
		fmt.Println("Silahkan masukkan data pabrikan terlebih dahulu")
	} else {
		var x string
		var i, k int
		var cek bool
		...
		cek = false
		fmt.Println("---------------------")
		fmt.Print("Masukkan nama pabrikan dari mobil yang akan ditambahkan: ")
		fmt.Scan(&x)
		fmt.Println()
		for i < a && !cek {
			if P[i].nama == x {
				cek = true
				if P[i].jumlahProduk < NMAX {
					P[i].jumlahProduk += 1
					k = P[i].jumlahProduk - 1
					fmt.Println("---------------------")
					fmt.Println("Masukkan data mobil dengan urutan Nama, Tahun Keluar, Penjualan Unit : ")
					fmt.Scan(&P[i].produk[k].nama, &P[i].produk[k].tahun, &P[i].produk[k].penjualan)
					fmt.Println("---------------------")
					fmt.Println("Data mobil sudah ditambahkan")
				} else {
					fmt.Println("---------------------")
					fmt.Println("Data mobil sudah penuh")
				}
			}
			i = i + 1
		}
