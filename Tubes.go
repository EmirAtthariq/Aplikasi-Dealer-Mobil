...
func inputData(a *int, P *pabrikan) {
	...
	for *a < NMAX && !cek {
		*a += 1
		...
		P[*a].nama = Pabrikan
		fmt.Println("---------------------")
		fmt.Print("Masukkan jumlah mobil buatan pabrikan tersebut: ")
		fmt.Scan(&jumlahMobil)
		if jumlahMobil > NMAX {
			jumlahMobil = NMAX
		}
		P[*a].jumlahProduk = jumlahMobil
		fmt.Println("---------------------")
		fmt.Println("Masukkan data mobil dengan urutan Nama, Tahun Keluar, Penjualan Unit : ")
		i = 0
		for i < jumlahMobil {
			fmt.Scan(&P[*a].produk[i].nama, &P[*a].produk[i].tahun, &P[*a].produk[i].penjualan)
			i += 1
		}
		i = 0
		for i < jumlahMobil {
			P[*a].penjualan += P[*a].produk[i].penjualan
			i += 1
		}
		fmt.Println("---------------------")
	...
	for i < *a && !cek {
		for j < P[i].jumlahProduk && !cek {
			if P[i].produk[j].nama == x {
				k = i
				l = j
				for l < P[k].jumlahProduk-1 {
					P[k].produk[l] = P[k].produk[l+1]
					l += 1
				}
				P[k].jumlahProduk = P[k].jumlahProduk - 1
				fmt.Println("---------------------")
